...
3. `plot_psi_trend(df_data, str_date_col, str_pred_col, arr_baseline, str_train_valid_end, str_freq='M', str_filename)` - PSI bar chart over time with threshold lines (orange=0.10, red=0.25). Single vertical line (black) marks the end of the training period (train+valid combined).
...
5. `plot_target_drift_over_time(df_data, str_date_col, str_target, str_pred_col, str_train_valid_end, str_freq='M', str_filename)` - Dual line plot: actual default rate vs mean predicted PD over time. Single vertical line marks training period end.

6. `plot_auc_over_time(df_data, str_date_col, str_target, str_pred_col, str_train_valid_end, str_freq='M', str_filename)` - AUC line plot over time. Single vertical line marks training period end.

7. `plot_calibration_over_time(df_data, str_date_col, str_target, str_pred_col, str_train_valid_end, str_freq='M', str_filename)` - Brier score line plot over time. Single vertical line marks training period end.
...
- Combine train and valid into the training set (the model was trained on combined train+valid)
- Generate predictions for train+valid (combined) and test
- Build monitoring DataFrames combining date, target, and predictions
- Combine into `df_all` for full timeline visualization
- Compute `str_train_valid_end` (YYYY-MM format) for the single vertical boundary line
- Train+valid predictions serve as the baseline population

### Analysis Sections

- Overall PSI with threshold interpretation
- PSI over time (monthly) on `df_all` with single boundary line at train+valid end. Training months serve as a control (should be near-zero PSI).
- CSI by feature (train+valid vs test clean data) with color-coded bar chart
- Target drift on `df_all`: actual vs predicted over time with boundary line
- AUC over time on `df_all` with boundary line
- Brier score over time on `df_all` with boundary line
